...
Installs and configures ESLint, Prettier, Husky pre-commit hooks, and lint-staged for the monorepo. Adds `pnpm lint` and `pnpm format` scripts, enforces TypeScript strict mode across all packages, and records the team's red/green/refactor workflow so later PRDs do not devolve into test-after-code work. This PRD runs in parallel with 00d (database setup) since they touch different files.
...
- [x] **Task 1: Install ESLint and Prettier** `[BD:STG-14]`
  - **Type**: task
...
- [x] **Task 2: Create ESLint flat config** `[BD:STG-15]`
  - **Type**: task
...
- [x] **Task 3: Install and configure Husky + lint-staged** `[BD:STG-16]`
  - **Type**: task
...
- [x] **Task 4: Add format scripts and TDD workflow docs** `[BD:STG-17]`
  - **Type**: task
  - **Do**: Add `format` script to root package.json: `prettier --write "**/*.{ts,tsx,json,md,yaml}"`. Add `format:check` script: `prettier --check "**/*.{ts,tsx,json,md,yaml}"` (used in CI). Create `docs/development-workflow.md` documenting the project's required red/green/refactor loop, the distinction between unit/integration/browser/device verification, and the rule that passing mocked tests alone does not complete a feature. Run `pnpm format` on the current codebase to normalize all files.
  - **Files**: `package.json`, `docs/development-workflow.md`, all source files (formatted in-place)
  - **Verify**: `pnpm format:check` exits 0 after running `pnpm format`
  - **Accept**: All current files pass prettier check; the development workflow doc exists and captures red/green/refactor expectations

- [x] **Task 5: Update manifest** `[BD:STG-18]`
  - **Type**: chore
  - **Do**: Open `prd-phases/manifest.md`. Find the registry entry for `00c`. Change `status: pending` to `status: complete`. Update Current State: "Last completed PRD" = `00c`, increment progress to `3 / 38 PRDs complete`.
  - **Files**: `prd-phases/manifest.md`
  - **Verify**: `grep "00c" prd-phases/manifest.md` shows `status: complete`
  - **Accept**: Manifest updated; progress = 3/38
