...
# Verify it works
bd list
bd create --id STG-1 -t task -d "Test task creation" "Test: verify bd works"
bd close STG-1
```
...
| Create task | `bd create --id STG-123 -t task -d "DESC" "TITLE"` |
| List tasks | `bd list` |
...
- [x] **Task 1: Verify bd is installed** `[BD:STG-1]`
  - **Type**: task
...
- [x] **Task 2: Initialize bd in the project** `[BD:STG-2]`
  - **Type**: task
...
- [x] **Task 3: Create foundation tracking tasks** `[BD:STG-3]`
  - **Type**: task
  - **Do**: Create bd tasks for each remaining foundation PRD so they are tracked in the system. Run these commands in order:
    ...
    bd create -t task -d "PostgreSQL + Drizzle schema + NextAuth (Auth.js) + USDA FDC dataset" "Foundation: Database setup (00d)"
...
- [x] **Task 4: Update manifest** `[BD:STG-4]`
  - **Type**: chore
  - **Do**: Open `prd-phases/manifest.md`. Find the registry entry for `00a`. Change `status: pending` to `status: complete`. Update the Current State section: set "Last completed PRD" to `00a`, update timestamp to current date/time, change progress to `1 / 38 PRDs complete`. Confirm `00b`'s `requires: 00a` is now satisfied.
  - **Files**: `prd-phases/manifest.md`
  - **Verify**: `grep "00a" prd-phases/manifest.md` shows `status: complete`
  - **Accept**: Manifest registry shows 00a complete; progress counter reads 1/38
