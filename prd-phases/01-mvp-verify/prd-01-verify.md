...
1. Add or refine failing persona flows before fixing any discovered gaps
...
- [ ] **Task 1: Author failing persona E2E flows** `[BD:STG-134]`
  - **Type**: task
  - **Do**: Create Playwright specs for Jordan (first-run onboarding to saved recipe), Maya (eco/dietary filters and offline recipe access), and Darius (planning to shared list to fulfillment). Ensure the specs fail until the full journeys are wired correctly.
  ...
  - **Verify**: The persona suite fails before fixes are applied
  - **Accept**: Full-user regression expectations are explicit before final MVP hardening

- [ ] **Task 2: Add shared fixture/reset support for persona runs** `[BD:STG-135]`
  - **Type**: task
...
- [ ] **Task 3: Run persona and offline regression passes** `[BD:STG-136]`
  - **Type**: task
...
- [ ] **Task 4: Verify browser console, network, and regression hygiene** `[BD:STG-137]`
  - **Type**: task
...
- [ ] **Task 5: Update manifest** `[BD:STG-138]`
  - **Type**: chore
