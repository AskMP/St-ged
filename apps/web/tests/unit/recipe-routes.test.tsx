...
import { RecipeLibrary, RecipeDetail, CookingView } from '../../src/routes/Recipes'
...
const mockRecipes = [
  {
    ...
    steps: ['Boil water and cook pasta.', 'Saute vegetables.', 'Combine and serve.'],
  },
  {
    id: 'r2',
    title: 'Lentil Soup',
...
beforeEach(() => {
  ...
  vi.mocked(apiClient.apiClient.recipes.scale).mockResolvedValue(mockRecipes[0])
  vi.mocked(apiClient.apiClient.recipes.substitute).mockResolvedValue({
    substitutions: ['spaghetti squash', 'rice noodles'],
  })
  vi.mocked(apiClient.apiClient.recipes.import).mockResolvedValue(mockRecipes[0])
})
...
describe('RecipeLibrary', () => {
  ...
  it('shows import error when import fails', async () => {
    vi.mocked(apiClient.apiClient.recipes.import).mockRejectedValue(new Error('Invalid URL'))
    render(
      <MemoryRouter>
        <RecipeLibrary />
      </MemoryRouter>
