...
const mockWeekPlan = {
  plan: { id: 'plan-1' },
  entries: [
    { id: 'e1', recipeId: 'r1', recipeTitle: 'Pasta Primavera', date: getMondayISO(), mealType: 'dinner' },
  ],
}
...
const mockGroceryList = {
  id: 'list-1',
  items: [
    { id: 'i1', listId: 'list-1', name: 'pasta 200g', checked: false, updatedAt: 0 },
    { id: 'i2', listId: 'list-1', name: 'zucchini', checked: true, updatedAt: 0 },
  ],
}

beforeEach(() => {
  vi.mocked(apiClient.apiClient.plans.getWeek).mockResolvedValue(mockWeekPlan)
  vi.mocked(apiClient.apiClient.plans.generateList).mockResolvedValue(mockGroceryList)
...
describe('Planning page', () => {
  ...
  it('shows connection badge', async () => {
    renderPlanning()
    await waitFor(() => {
  ...
  it('queues sync when generate-list fails and shows badge', async () => {
    vi.mocked(apiClient.apiClient.plans.generateList).mockRejectedValue(new Error('offline'))
    renderPlanning()
    await waitFor(() => screen.getByTestId('generate-list-btn'))
    fireEvent.click(screen.getByTestId('generate-list-btn'))
    await waitFor(() => {
      expect(syncQueue.enqueue).toHaveBeenCalledWith('generate-list', expect.any(Object))
    })
    await waitFor(() => {
