...
const fulfillmentRouter = new Hono<{ Variables: AppVariables }>()
  .use("*", requireAuth)
  ...
  .get("/providers", async (c) => {
    ...
    if (providers.includes("instacart-official"))
      defaultProv = "instacart-official";
  ...
  .post("/link", async (c) => {
    ...
    const user = c.get("user");
    const link = await fulfillmentService.generateLink(
      listId,
      user.id,
      provider,
    );
