...
import { createHousehold, joinHousehold } from '../../src/services/household-service'
...
describe('household ops routes', () => {
  it('allows posting a cost entry and retrieving history', async () => {
    ...
    const hid = h.id
    // add second member via invite code
    await joinHousehold(h.inviteCode, 'userB')

    const postRes = await app.request(`http://localhost/api/households/${hid}/costs`, {
      method: 'POST',
    ...
    const getRes = await app.request(`http://localhost/api/households/${hid}/costs`, {
      method: 'GET',
  ...
  it('lets clients configure rotation and fetch assignments', async () => {
    ...
    const postRes = await app.request(`http://localhost/api/households/${hid}/rotation`, {
      method: 'POST',
      headers: { 'content-type': 'application/json', 'x-test-user-id': 'userX' },
      body: JSON.stringify({ frequency: 'weekly', members: ['userX','userY','userZ'], startDate: '2025-01-01' }),
    })
    expect(postRes.status).toBe(200)
    const settings = await postRes.json()
    expect(settings.frequency).toBe('weekly')

    const getRes = await app.request(`http://localhost/api/households/${hid}/rotation`, {
      method: 'GET',
    ...
    const assignsRes = await app.request(`http://localhost/api/households/${hid}/rotation/assignments?weeks=3`, {
      method: 'GET',
      headers: { 'x-test-user-id': 'userX' },
    })
