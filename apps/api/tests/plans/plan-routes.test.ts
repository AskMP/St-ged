...
describe("plan routes", () => {
  it("can create week plan, add entry, copy week, and generate list", async () => {
    ...
    const fetchRes = await app.request(
      `http://localhost/api/households/${hid}/plans/week?start=${week}`,
      {
    ...
    // add a recipe entry (must be a real DB recipe for FK constraint)
    const recipe = await createTestRecipe("Burger");
    const addRes = await app.request(
      `http://localhost/api/plans/${base.plan.id}/entries`,
      {
        method: "POST",
        headers: {
          "content-type": "application/json",
    ...
    const fetch2 = await app.request(
      `http://localhost/api/households/${hid}/plans/week?start=${week}`,
      {
        headers: { "x-test-user-id": "test-user" },
      },
    ...
    const copyRes = await app.request(
      `http://localhost/api/households/${hid}/plans/copy`,
      {
        method: "POST",
        headers: {
          "content-type": "application/json",
    ...
    const genRes = await app.request(
      `http://localhost/api/plans/${base.plan.id}/generate-list`,
      {
        method: "POST",
        headers: { "x-test-user-id": "test-user" },
      },
    ...
    // fetch items via GET list route (recipe has no ingredients so list may be empty)
    const itemsRes = await app.request(
      `http://localhost/api/lists/${list.id}`,
      {
        headers: { "x-test-user-id": "test-user" },
      },
    );
